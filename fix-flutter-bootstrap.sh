...
# Fix Flutter Bootstrap Script
# Prevents duplicate Flutter initialization by modifying flutter_bootstrap.js

echo "🔧 Fixing Flutter Bootstrap Duplicate Initialization..."

BOOTSTRAP_FILE="build/web/flutter_bootstrap.js"

if [ ! -f "$BOOTSTRAP_FILE" ]; then
    echo "❌ flutter_bootstrap.js not found at $BOOTSTRAP_FILE"
    exit 1
fi

# Create backup
cp "$BOOTSTRAP_FILE" "$BOOTSTRAP_FILE.backup"
echo "📁 Created backup: $BOOTSTRAP_FILE.backup"

# Apply the fix to prevent duplicate initialization
...
// Flutter Bootstrap Initialization Safeguard
// Prevents duplicate initialization that causes issues with route navigation

console.log('[Flutter Bootstrap] Initializing with safeguards...');

if (!window._flutter) {
  window._flutter = {};
}

// Force HTML renderer in buildConfig to override CanvasKit
...
if (window.initializeFlutterSafely) {
  // Use the main safeguard system from index.html
  console.log('[Flutter Bootstrap] Using main safeguard system');
  ...
  // Fallback: implement basic safeguard logic
  console.log('[Flutter Bootstrap] Using fallback safeguard');
  
  if (!window.flutterInitializationState) {
  ...
  if (!window.flutterInitializationState.started) {
    ...
    window.flutterInitializationState.method = 'flutter_bootstrap.js';
    ...
    console.warn('[Flutter Bootstrap] Skipping initialization - already started via:', 
                 window.flutterInitializationState.method);
  }
}

console.log('[Flutter Bootstrap] Initialization safeguards applied');
EOF

echo "✅ Flutter bootstrap fix applied successfully"
echo "🔍 Safeguards implemented:"
echo "   - Prevents duplicate initialization"
echo "   - Forces HTML renderer"
echo "   - Provides fallback logic"
echo "   - Adds comprehensive logging"

# Verify the file was written correctly
if [ -f "$BOOTSTRAP_FILE" ] && grep -q "Flutter Bootstrap Initialization Safeguard" "$BOOTSTRAP_FILE"; then
    echo "✅ Verification successful: flutter_bootstrap.js fixed"
else
    echo "❌ Verification failed: fix may not have applied correctly"
    exit 1
fi

echo "🚀 Flutter bootstrap is now protected against duplicate initialization!"