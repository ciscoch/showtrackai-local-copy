...
class AnimalService {
  ...
  Future<Animal> updateAnimal(Animal animal) async {
    try {
      ...
      // Step 5: Prepare update data (excluding non-updatable fields)
      final updateData = animal.toJson();
      // Remove fields that shouldn't be updated (RLS policy violation)
      updateData.remove('id');        // Primary key shouldn't be updated
      updateData.remove('user_id');   // RLS policy prevents changing ownership
      updateData.remove('created_at'); // Creation timestamp is immutable
