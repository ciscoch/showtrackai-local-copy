...
class ApiConfig {
  ...
  static String get _functionsBase {
    ...
    if (isDevelopment && _isRelative(base)) {
      return 'http://localhost:9999$base'; // netlify dev
    }
    return base;
  }
  ...
  // Netlify Functions endpoints (strings only; no absolute domains):
  static String get journalCreate             => fn('journal-create');
  static String get journalUpdate             => fn('journal-update');
  static String get journalDelete             => fn('journal-delete');
  ...
  static Map<String, String> getHeadersWithTrace({String? authToken, String? traceId}) {
    final headers = getDefaultHeaders(authToken: authToken);
    if (traceId != null && traceId.isNotEmpty) headers['X-Trace-ID'] = traceId;
    return headers;
  }
